...
#### 📶 玩家 RTT 计分板（多人判定延迟补偿用）—— **已搁置/回退（2026-09-26）**

- 原先：每 20 刻把每位在线玩家的 RTT（毫秒）写进计分板 `net`，供数据包把判定箱回退到这个玩家「眼睛看到的位置」。
- **为什么搁置**：实测（拿编辑器无补偿当对照）结论是「**没什么用，手感还更差**」——
  - RTT 只有 20~50ms（≈0.4~1 刻），而实际感知滞后是「**几刻**」量级 ⇒ 滞后的大头是与 ping 无关的**稳定**成分（客户端渲染/插值节奏）；
  - 按 RTT 补的那一点点还在 **1↔2 刻之间飘**（keep-alive 采样抖动 × 整刻量化）⇒ 恒定偏差玩家能适应、飘的不能。
- **现状**：mod 不再写 `net`（`PlayerPingSync` 已删）；数据包侧补偿代码保留但**恒等于不补偿**（读不到 `net` ⇒ `#st_lag = 0`），总开关 `judge_lag_comp` 默认改为 **0**。
- 待办与完整结论：数据包文档 `节奏地图开发文档/todo.md`。
